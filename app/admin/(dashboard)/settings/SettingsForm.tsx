...
export default function SettingsForm({ initial }: { initial: SiteSettings }) {
  ...
  return (
    <form action={formAction} className="grid grid-cols-1 items-start gap-6 xl:grid-cols-[1fr_380px]">
      <input type="hidden" name="logoUrl" value={logoUrl} />
      <input type="hidden" name="socialLinks" value={JSON.stringify(socialLinks)} />

      <div className="flex flex-col gap-5 rounded-2xl border border-navy-900/8 bg-white p-6">
        <h2 className="text-sm font-bold uppercase tracking-wider text-navy-900/40">Contact details</h2>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col gap-3 rounded-2xl border border-navy-900/8 bg-white p-6">
          ...
          <div className="flex h-20 w-full items-center justify-center overflow-hidden rounded-lg border border-navy-900/8 bg-navy-50/40 p-3">
            {logoUrl && (
              <Image src={logoUrl} alt="Logo preview" width={200} height={80} className="h-full w-full object-contain" />
            )}
          </div>
          <label className="flex w-fit cursor-pointer items-center gap-2 rounded-full border border-navy-900/15 bg-white px-4 py-2 text-sm font-semibold text-navy-900 transition-colors hover:border-orange-300 hover:text-orange-600">
        ...
        <div className="flex flex-col gap-4 rounded-2xl border border-navy-900/8 bg-white p-6">
